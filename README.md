...
The integration's Active switch is the master switch. When it is off, the
bundle does not register form/report/webhook features, send DOI messages,
accept DOI links, or apply queued contact changes.
...
### Optional: delayed DOI processing via Messenger

Mautic 5+ removed `QueueBundle`; this plugin now uses Symfony Messenger for DOI confirmation processing.

By default Mautic uses `sync://` transport, so DOI actions are handled immediately.
If you want delayed processing (for stronger protection against automated email link scanners), route this message class to an async transport:
...
Maintained by Alexander Zlobin.
