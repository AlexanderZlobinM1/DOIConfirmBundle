...
class FormSubscriber implements EventSubscriberInterface
{
    ...
    private function shouldEmailBeSended($lead)
    {
        ...
        foreach ($lead->getDoNotContact() as $dnc) {
            ...
            if (DNC::UNSUBSCRIBE === $reason && 'email' === $channel) {
                return true;
            }

            if (DNC::BOUNCED === $reason && 'email' === $channel) {
                return false;
            }

            if (DNC::MANUAL === $reason && 'email' === $channel) {
                return true;
            }
    ...
    private function sendDoiEmail($lead, $config, $doidata, $tokens, $submissionId)
    {
        ...
        if (
            (empty($currentLead['email']) || !is_string($currentLead['email']))
            && !empty($config['alternative_email_field'])
            && !empty($currentLead[$config['alternative_email_field']])
        ) {
            $currentLead['email'] = $currentLead[$config['alternative_email_field']];
        }

        if (
            (empty($currentLead['email']) || !is_string($currentLead['email']))
            && isset($currentLead['email_validate'])
            && $currentLead['email_validate']
        ) {
